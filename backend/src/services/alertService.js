 ...
 * Rules: Allow multiple alerts. Trigger direction is explicit or inferred.
 */
async function createAlert(userId, targetPrice, currentPrice, explicitDirection = null) {
    // Safety: ensure numbers
    const target = parseFloat(targetPrice);
    const current = currentPrice ? parseFloat(currentPrice) : null;

    // Check for existing active, untriggered alert with SAME target price AND direction
    // Logic: If user sets SAME target, we check if they already have an active one.
    // Ideally we should allow it if direction is different, but for simplicity let's just check target for now or just allow it.
    // Let's prevent EXACT duplicates (same target and direction).
    ...
    const checkRes = await db.query(
        'SELECT id FROM alerts WHERE user_id = $1 AND target_price = $2 AND direction = $3 AND active = true AND triggered = false',
        [userId, targetPrice, direction]
    );

    if (checkRes.rows.length > 0) {
        const error = new Error('Alert for this price and direction already active');
    ...
    const insertRes = await db.query(
        `INSERT INTO alerts (user_id, metal, target_price, direction, active, triggered)
         ...
         RETURNING id, metal, target_price, direction, active`,
        [userId, targetPrice, direction]
    );
